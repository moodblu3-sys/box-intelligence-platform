...
Knowledge Deskは、顧客企業の情シス部門向け社内問い合わせAIアプリである。社員からの問い合わせに対して、Box、Jiraに分散したナレッジを横断検索し、根拠付きの回答を返す。SharePointは復旧後に追加する。
...
- Onyx: Box、Jiraなどのデータソースを接続し、検索・RAG基盤を提供する。SharePointも同じ方式で追加できる。
- Knowledge Desk API: 情シス問い合わせ業務に必要な回答形式、confidence判定、エスカレーション判定、Jira起票を担当する。

今回の実装では `OnyxClient` interfaceを定義し、`MockOnyxClient`でBox、SharePoint、Jira由来の検索結果を返す。`KNOWLEDGE_DESK_ONYX_MODE=real` に切り替えると、同じinterfaceのまま実Onyx APIへ接続できる。実デモではBox + Jiraを確認済み、SharePointは後回し。
...
Knowledge Deskでは、SharePointを「社員に案内する具体手順」として引用する。今回はSharePointにアクセスできないため、デモでは見せず、復旧後の追加対象とする。
...
3. Knowledge Desk APIがOnyxを通じてBox、Jiraを検索する。SharePointは復旧後に追加する。
...
- Boxの正式ルールだけでなく、Jiraの過去解決履歴も参照する。SharePoint FAQは復旧後に追加できる。
